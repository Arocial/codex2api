...
The proxy normalizes the following request fields:
...
| `parallel_tool_calls` | Defaults to `false`. |
| `include` | Defaults to `["reasoning.encrypted_content"]`. |
| `prompt_cache_key` | Defaults to the effective session ID. |
| `client_metadata` | Codex identity fields are generated by the proxy; unrelated client fields are preserved. |

#### Codex session identity

Send `X-Session-Id` to keep multiple requests in the same Codex thread:

```sh
curl -N http://127.0.0.1:3402/v1/responses \
  -H 'Authorization: Bearer replace-with-a-long-random-secret' \
  -H 'Content-Type: application/json' \
  -H 'X-Session-Id: 019f463d-04e3-7fd1-943d-c186fe8f79db' \
  -d '{"model":"gpt-5.5","input":"Continue the task."}'
```

`session-id` is accepted as a fallback alias. `X-Session-Id` takes precedence
when both are present. If neither is supplied, the proxy generates a UUIDv7
session ID and returns it in the `x-session-id` response header.

For each request, the proxy derives a coherent set of Codex identity fields:

- `session-id`, `thread-id`, and `x-client-request-id` use the effective session ID.
- `x-codex-window-id` uses `<session-id>:0`.
- `turn_id` is a new UUIDv7.
- `prompt_cache_key` defaults to the session ID.
- The same turn metadata JSON is sent in both the
  `x-codex-turn-metadata` header and `client_metadata`.

A stable installation UUID is read from or created at
`$CODEX_HOME/installation_id`. The proxy also sends the configured client
version, a Codex TUI-compatible User-Agent, `originator: codex-tui`, and
`Accept: text/event-stream` to the Responses backend.
...
The proxy reproduces the stable installation identity, session/thread/window
mapping, per-turn identity, compatibility metadata, client version,
originator, and the common Responses body defaults. It is still **not**
byte-for-byte indistinguishable from a direct Codex CLI session:

- `x-codex-beta-features` depends on the CLI's enabled feature set.
- `x-openai-internal-codex-responses-lite` is model-capability dependent and
  is not guessed by the proxy.
- `x-oai-attestation` requires a genuine device attestation.
- Request compression, TLS characteristics, workspace Git metadata, terminal
  details, and client-supplied instructions/tools may differ.

The proxy deliberately does not copy browser cookies or invent attestation
values. Those fields are not required for OAuth-authenticated Responses API
operation and fabricating them would reduce compatibility.
